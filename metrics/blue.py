...
"""
from: from allennlp.training.metrics import BLEU
modify: 
    {f"BLEU-{len(self._ngram_weights)}": bleu}
"""


def get_bleu_1to4(exclude_indices):
    """
    usage:
        self._tensor_based_metrics = get_bleu_1to4(exclude_indices=exclude_indices)
    """
    ngram_dict = {
        1: (1,),
        2: (1 / 2, 1 / 2),
    ...
    bleus = [BLEU(ngram_weights=ngram_dict[i], exclude_indices=exclude_indices) for i in range(1, 5, 1)]
    return bleus
...
class BLEU(Metric):
    ...
    def get_metric(self, reset: bool = False) -> Dict[str, float]:
        ...
        ngram_scores = (
            weight
            * (math.log(self._precision_matches[n] + 1e-13) - math.log(self._precision_totals[n] + 1e-13))
        ...
        if reset:
            self.reset()
        return {f"BLEU-{len(self._ngram_weights)}": bleu}
