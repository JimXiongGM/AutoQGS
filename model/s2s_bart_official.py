...
from metrics import PaperBLEU
from model.my_beam_search import BeamSearch, LengthNormalizedSequenceLogProbabilityScorer
...
class Bart(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if tuning:
            from model.my_modeling_bart_tuning import BartForConditionalGeneration

            self.bart = BartForConditionalGeneration.from_pretrained(model_name)
        else:
            from model.my_modeling_bart import BartForConditionalGeneration
        ...
        # reward: group by id and choose the max logit
        self._reward_mode = kwargs.get("reward_mode", False)
        if self._reward_mode:
            self._best_pred = {}

        self.decoder_instructions = kwargs.get("decoder_instructions", False)
        self.decoder_prompt = kwargs.get("decoder_prompt", "")
        self.beam_length_penalty = kwargs.get("beam_length_penalty", 1.0)
        if self.decoder_instructions and self.decoder_prompt:
            self._decoder_start_ids = self.tok.encode("<s> " + self.decoder_prompt, add_special_tokens=False)
        else:
            self._decoder_start_ids = [self.bart.config.decoder_start_token_id or self._start_id]
        ...
        # 增加长度惩罚
        beam_search_extras["final_sequence_scorer"] = LengthNormalizedSequenceLogProbabilityScorer(
        ...
        if encoder is not None:
            assert encoder.get_input_dim() == encoder.get_output_dim() == self.bart.config.hidden_size
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.training:
            ...
            labels[target_ids[:, 1:] == self._pad_id] = -100
            ...
            beam_result = self._beam_search.search(initial_decoder_ids, inital_state, self.take_step)

            predictions, final_scores = beam_result
            max_pred_indices = (
                final_scores.argmax(dim=-1).view(-1, 1, 1).expand(-1, -1, predictions.shape[-1])
            )
            predictions = predictions.gather(dim=1, index=max_pred_indices).squeeze(dim=1)

            # reward
            # if self._reward_mode:
            #     # (id, beam_score, predictions)
            #     _best_predictions = []
            #     best_beam_logits = final_scores.max(dim=-1)[0]
            #     batch_ids = [i["ID"].split("@beam")[0] for i in metadata]
            #     assert len(best_beam_logits)==len(batch_ids)==len(predictions)
            #     for _id,_logit,_pred in zip(batch_ids,best_beam_logits,predictions):
            #         _logit,_pred = _logit.detach(),_pred.detach()
            #         if _id not in self._best_pred or _logit > self._best_pred[_id]["logit"]:
            #             self._best_pred[_id] = {"logit":_logit,"prediction":_pred}
            #             _best_predictions.append(_pred)
            #         else:
            #             _best_predictions.append(self._best_pred[_id]["prediction"])

            #     predictions=torch.stack(_best_predictions)

            # self._rouge(predictions, target_ids)
            outputs["pred_tgt"] = predictions
            outputs["pred_probs"] = final_scores.gather(dim=-1, index=max_pred_indices[..., 0]).squeeze(
                dim=-1
            )

            _predictions = self.tok.batch_decode(predictions, skip_special_tokens=True)
            if targets is not None:
                _target_ids = self.tok.batch_decode(target_ids, skip_special_tokens=True)
                self._bleu(
                    self._clean_underline(_predictions),
                    self._clean_underline(_target_ids),
                )

            # outputs["pred_tgt"] = _predictions
            self.make_output_human_readable(outputs)

        return outputs

    def _clean_underline(self, x: list):
    ...
    def take_step(
        ...
        decoder_cache = None
        decoder_cache_dict = {
            k: state[k].contiguous() for k in state if k not in {"input_ids", "input_mask", "encoder_states"}
        }
    ...
    def make_output_human_readable(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        """
        predictions = output_dict["pred_tgt"]
        ...
        output_dict["predicted_tokens_bart"] = predicted_tokens  # type: ignore
        ...
        output_dict["predicted_text"] = [
            i.replace(self.decoder_prompt, "").replace("_", " ") for i in output_dict["predicted_text"]
        ]

        # for eval
        # output_dict["predicted_tokens"] = [
        #     nltk_tokenizer(text=i) for i in output_dict["predicted_text"]
        # ]
