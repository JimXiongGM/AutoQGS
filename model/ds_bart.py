...
from common.common_utils import colorful
...
"""
DS版的bart没有metrics，直接计算loss，print wqcwq中遇到的schema即可
"""
...
class DSBart(Model):
        ...
        **kwargs,
        ...
        if self.training:
            ...
            try:
                ...
                print(
                    colorful("bart_outputs.logits", color="red"),
                    bart_outputs.logits.shape,
                )
                print(colorful("target_ids.shape", color="red"), target_ids.shape)
                raise e
        ...
        # Use decoder start id and start of sentence to start decoder
            ...
            predictions, beam_logits = beam_result
            outputs["predictions"] = predictions.tolist()
            outputs["beam_logits"] = beam_logits

            # batch_beams = []
            # for bsz in range(predictions.shape[0]):
            #     beams = []
            #     for bmidx in range(predictions.shape[1]):
            #         beam = predictions[:, bmidx, :]
            #         tmp = {"pred_tgt": beam}
            #         self.make_output_human_readable(tmp)
            #         beams.append(tmp["predicted_text"][0])
            #     batch_beams.append(beams)
            # outputs["beam_search"] = batch_beams
    ...
    def make_output_human_readable(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        """
        if "pred_tgt" not in output_dict:
            return output_dict
