...
def masked_softmax(
    ...
    """
    `torch.softmax(vector)` does not work if some elements of `vector` should be
...
def combine_tensors_and_multiply(
    ...
    for result_piece in to_sum[1:]:
        # my fixbug! missing `.squeeze(1)` will return wrong shape [bsz, bsz, matrix_len]
        result = result + result_piece.squeeze(1)
    return result
...
def _get_combination_and_multiply(
    ...
    """
    核心操作：torch.matmul(tensors_1, weight)
    说明：
        1. x:[bsz, dim]  y: [bsz, y_len, dim]
        2. x @ weight = [bsz, dim] @ [dim,1] = [bsz, 1]
    """
    if combination.isdigit():
        index = int(combination) - 1
